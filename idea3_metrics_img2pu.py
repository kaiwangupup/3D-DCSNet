...
def fetch_batch(models, indices, batch_num, batch_size):
	...
	return np.array(pcl_path_gt), np.array(batch_ip), np.array(batch_gt)
...
def calculate_metrics(models, indices, pcl_gt_1K_scaled, pred_scaled):
	...
	for cnt in iters:
		...
		pcl_path, batch_ip, batch_gt_1K = fetch_batch(models, indices, cnt, BATCH_SIZE)
		...
		if FLAGS.visualize:
			for i in xrange(BATCH_SIZE):
				...
				show3d_balls.showpoints(_pr_scaled_icp[i][:1024], ballradius=4)
				print 'PR:', _pr_scaled_icp[i].shape

				print 'Displaying Gt scaled icp 1k'
				show3d_balls.showpoints(_gt_scaled_1K[i][:1024], ballradius=4)
...
if __name__ == '__main__':
	...
	with tf.variable_scope('generator') as scope:
		...
		up_l2_points, l1_points = get_local_features(reconstr, is_training=False, scope=scope,
