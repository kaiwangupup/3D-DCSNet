...
def showpoints(xyz,c0=None,c1=None,c2=None,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(255,255,255),normalizecolor=False,ballradius=10):
	global showsz,mousex,mousey,zoom,changed
	# xyz=xyz-xyz.mean(axis=0)
	radius=((xyz**2).sum(axis=-1)**0.5).max()
	xyz/=(radius*2.2)/showsz
	if c0 is None:
		c0=np.zeros((len(xyz),),dtype='float32')+50 #255 #205 #205 green #50  #50 #150 purple
	if c1 is None:
		c1=np.zeros((len(xyz),),dtype='float32') + 50  
	if c2 is None:
		c2=np.zeros((len(xyz),),dtype='float32') + 150  
	if normalizecolor:
