...
def get_local_features(point_cloud, is_training, scope, bradius = 1.0, reuse=None, use_rv=False, use_bn = False,use_ibn = False,
    ...
    with tf.variable_scope(scope,reuse=reuse) as sc:
        ...
        l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=num_point, radius=bradius*0.05,bn=use_bn,ibn = use_ibn,
                                                           nsample=32, mlp=[32, 32, 64], mlp2=None, group_all=False,
                                                           is_training=is_training, bn_decay=bn_decay, scope='layer1')
        print 'get_local_features l1_points:', l1_points.get_shape
        ...
        # l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=num_point/4, radius=bradius*0.3,bn=use_bn,ibn = use_ibn,
        #                                                     nsample=32, mlp=[128, 128, 256], mlp2=None, group_all=False,
        #                                                     is_training=is_training, bn_decay=bn_decay, scope='layer3')
        ...
        up_l2_points = pointnet_fp_module(l0_xyz, l2_xyz, l1_points, l2_points, [64], is_training, bn_decay,
                                       scope='fa_layer3',bn=use_bn,ibn = use_ibn)
        print 'get_local_features up_l2_points:', up_l2_points.get_shape
        
        return up_l2_points, l1_points
...
def concat_features(features, up_l2_points, l1_points, global_feature, is_training, scope, reuse=None,
        ...
        with tf.variable_scope(scope,reuse=reuse):
            ...
            for i in range(up_ratio):
                ...
                new_points_list.append(new_points)
            net = tf.concat(new_points_list,axis=1)
            print 'concat net:', net.get_shape
        return net

def concat_features_compare(up_l2_points, l1_points, is_training, scope, reuse=None,
                  bn_decay=None):
        ###concat feature
        with tf.variable_scope(scope,reuse=reuse):
            up_ratio = 2
            new_points_list = []
            for i in range(up_ratio):
                concat_feat = tf.concat([up_l2_points, l1_points], axis=-1)
                concat_feat = tf.expand_dims(concat_feat, axis=2)
                concat_feat = conv2d(concat_feat, 256, [1, 1],
                                              padding='VALID', stride=[1, 1],
                                              bn=False, is_training=is_training,
...
# def concat_features(features, global_feature, is_training, scope, reuse=None,
#                   bn_decay=None):
#         ###concat feature
#         with tf.variable_scope(scope,reuse=reuse):
#             up_ratio = 2
#             new_points_list = []
#             for i in range(up_ratio):
#                 concat_feat = tf.concat([features, global_feature], axis=-1)
#                 concat_feat = tf.expand_dims(concat_feat, axis=2)
#                 concat_feat = conv2d(concat_feat, 256, [1, 1],
#                                               padding='VALID', stride=[1, 1],
#                                               bn=False, is_training=is_training,
...
def concat_features_wrong(features, up_l2_points, l1_points, global_feature, is_training, scope, reuse=None,
                  bn_decay=None):
        ###concat feature
        with tf.variable_scope(scope,reuse=reuse):
            up_ratio = 2
...
def feature_extraction(inputs, scope='feature_extraction', is_training=True, bn_decay=None):
    ...
    with tf.variable_scope(scope, reuse=False):
        ...
        print 'feature_extraction l0 shape is:', l0_features.get_shape
        ...
        print 'feature_extraction l1 shape is:', l1_features.get_shape
        ...
        print 'feature_extraction l2 shape is also features:', l2_features.get_shape
...
def dense_conv(feature, n=3,growth_rate=64, k=16, scope='dense_conv',**kwargs):
    with tf.variable_scope(scope, reuse=False):
        print 'in dense conv feature old shape is:',feature.get_shape
        y, idx = get_edge_feature(feature, k=k, idx=None)  # [B N K 2*C]
        print 'dense conv y old shape is:',y.get_shape
        for i in range(n):
            if i == 0:
                y = tf.concat([
                ...
                print i
                print 'this y shape is',y.get_shape
            elif i == n-1:
                y = tf.concat([
        ...
        print 'dense conv y new shape is:',y.get_shape
        return y,idx
