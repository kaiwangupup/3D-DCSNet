...
def get_epoch_loss(val_models, val_pair_indices):
	...
	for b in xrange(batches):
		...
		runlist = [loss, chamfer_distance_rimg_scaled, dists_forward_rimg_scaled, dists_backward_rimg_scaled, emd_scaled]
		_l, C, F, B, E = sess.run(runlist, feed_dict={pcl_gt:batch_gt, img_inp:batch_ip})
		_summary_losses = [F, B, C, _l, E]
...
if __name__ == '__main__':
	...
	with tf.variable_scope('generator') as scope:
		...
		print '+' * 30
		up_l2_points, l1_points = get_local_features(reconstr, is_training=True, scope=scope,
	...
	# Calculate EMD
	# emd = tf.reduce_mean(get_emd_metrics(pcl_gt, reconstr_img, BATCH_SIZE, GT_PCL_SIZE))
	# Calculate EMD scaled
	emd_scaled = tf.reduce_mean(get_emd_metrics(pcl_gt_scaled, reconstr_img_scaled, BATCH_SIZE, GT_PCL_SIZE))
	...
	if FLAGS.loss == 'chamfer':
		...
		loss = chamfer_distance_rimg_scaled + emd_scaled
	elif FLAGS.loss == 'emd':
		loss = emd_scaled
	...
	ph_summary = [ph_dists_forward, ph_dists_backward, ph_dists_chamfer, ph_loss, ph_emd]
