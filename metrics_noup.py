...
def calculate_metrics_pix3d(models, batches, pcl_gt_1K_scaled, pred_scaled, indices=None):
	# batches = len(indices) / BATCH_SIZE
	if FLAGS.visualize:
		iters = range(batches)
	else:
	...
	for cnt in iters:
		...
		_gt_scaled_1K, _pr_scaled = sess.run(
			[pcl_gt_1K_scaled, pred_scaled],
			feed_dict={pcl_gt_1K: batch_gt, img_inp: batch_ip}
		)
...
if __name__ == '__main__':
	...
	out = tf.reshape(reconstr, (BATCH_SIZE, GT_PCL_SIZE, 3))
...
# *******************************************************
		...
		if FLAGS.dataset == 'shapenet':
			...
			batches = len(models)
			calculate_metrics_pix3d(models, batches, pcl_gt_1K_scaled, reconstr_img_scaled)
